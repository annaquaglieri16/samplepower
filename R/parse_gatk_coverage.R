...
if(TCGA){
  ...
  names_samples <- unique(truth_set$SampleName)
}
...
# I ran the depth of coverage for GATK on more samples that the ones I am greping for (aka the ones present in the truth set)
# This means that some columns won't be present in my final set
cov_long_alt_depth1 <- cov_long_alt_depth %>% dplyr::left_join(create_match_names_alt) %>%
  dplyr::filter(!is.na(SampleName)) %>% dplyr::select(- base_alt_counts)

cov_long_tot_depth1 <- cov_long_tot_depth %>% dplyr::left_join(create_match_names_tot) %>%
  dplyr::filter(!is.na(SampleName)) %>% dplyr::select(- base_tot_counts)

cov_long <- cov_long_alt_depth1 %>% dplyr::left_join(cov_long_tot_depth1)
...
cov_long_infos1$tot_depth_GATK <- as.numeric(as.character(cov_long_infos1$tot_depth_GATK))

# Extract correct Alternative depth
...
for(i in 1:nrow(cov_long_infos1)){
  ...
  cov_long_infos1$alt_depth_GATK[i] <- as.numeric(as.character(gsub(paste0(allele_alt,":"),"",allele_alt_dep)))
}

cov_long_infos1$VAF_GATK <- cov_long_infos1$alt_depth_GATK/cov_long_infos1$tot_depth_GATK
